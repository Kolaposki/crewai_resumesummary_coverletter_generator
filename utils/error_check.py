        ...
        if create_pdf:  # Only proceed if create_pdf is True
            try:
                ...
                raise EnvironmentError(
                    "LibreOffice is not installed or not accessible from the command line. Please ensure LibreOffice is installed and 'soffice' command can be run.")
        else:
        ...
        if not scrape_config.get("perform_scrape", False):
            ...
            if not os.path.exists(output_file_path):
                raise FileNotFoundError(
                    "output.json does not exist. Please enable scraping or manually provide the file in the root directory.")
